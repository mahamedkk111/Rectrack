...
def add_transaction_db(customer_name, ttype, amount, note, dt):
    ...
    if not row:
        ...
        return False
    customer_id = row[0]
    c.execute("INSERT INTO transactions (customer_id, type, amount, note, dt) VALUES (?, ?, ?, ?, ?)",
              (customer_id, ttype, amount, note, dt.strftime("%Y-%m-%d %H:%M:%S")))
    conn.commit()
    conn.close()
    return True
...
def edit_transaction_db(trans_id, amount, note):
    conn = get_db()
    conn.execute("UPDATE transactions SET amount=?, note=? WHERE id=?", (amount, note, trans_id))
    conn.commit()
    conn.close()
...
def get_total_balance():
    names = get_customers()
    return sum(get_balance(name) for name in names)

def get_sorted_customers_by_balance():
    # Returns list of (name, balance), sorted by balance descending
    names = get_customers()
    return sorted([(name, get_balance(name)) for name in names], key=lambda x: x[1], reverse=True)
...
class MainMenu(Screen):
    ...
    def add_transaction(self, ttype):
        ...
        try:
            ...
            return
        add_transaction_db(name, ttype, amount, note, datetime.now())
        self.spinner.text = "Select Customer"
        self.bal_label.text = ""
